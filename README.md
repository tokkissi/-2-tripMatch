...
- 기술 스택
  <center>
    <img
      src="https://res.cloudinary.com/dnow6qfd8/image/upload/v1672301273/2022-12-29_17_03_48_eadleu.png"
      width="60%"
    />
  </center>
...
- 주요 기능 및 서브 기능 소개
  - 유저 관련 기능
    1. 로그인
      1. 비밀번호 찾기
    2. 회원가입(필수 값 * 표시)
      1. 이메일 인증 및 인증번호 확인* (이메일 형식 검증)
      2. 닉네임* (중복 가능) (2자 이상 / 8자 이내 / 특문 제외)
      3. 비밀번호* (8자 이상 / 영어, 숫자, 특문 포함)
      4. 성별*, 나이*
      5. 자기소개 (100자 이내)
    3. 마이페이지
      1. 유저의 동행 횟수, 점수 조회
      2. 유저 정보 수정
        1. 프로필 사진, 비밀번호, 나이, 자기소개 수정 가능
        2. 이메일,닉네임,성별 수정 불가 
      3. 작성 게시글 내역 조회
        1. 모집중, 모집 마감 수정
        2. 제목, 지역, 기간, 모집상태 조회
      ...
      5. 신청 받은 내역 조회
        1. 수락, 거절
      6. 신청한 내역 조회
        1. 신청 상태(대기중, 거절, 수락) 조회
        2. 대기중 시 신청 취소 가능
        3. 수락 시 연락 수단 조회 가능
          1. 수락 후 여행 기간이 끝난 뒤에 7일 이내 리뷰 작성
          2. 모달을 통해 동행자에 대한 별점을 남길 수 있음
  - 위시리스트
    1. 좋아요 누른 글 조회
    2. 좋아요 누른 글 개수 조회
    3. 좋아요 취소
  ...
  - 동행 게시판 (동행 모집 글 게시판)
    1. 지역별 게시글 전체 조회
      ...
      2. 희망(위시 리스트) 모집글 표시
    2. 게시글 작성
      1. 지역, 모집 인원, 여행 기간, 희망 성별 및 나이대, 관련 사진 입력 가능
    3. 게시글 상세 조회
      1. 모달을 통해 작성자 상세 정보 조회
      2. 동행 신청
      3. 댓글 작성, 수정, 삭제 가능
  ...
  - 관광 정보 페이지
    1. 축제 정보 목록 조회
      1. 축제 클릭 시 모달을 통해 상세 정보 조회
    2. 숙박 정보 목록 조회
      1. 숙소 클릭 시 모달을 통해 상세 정보 조회
  - 관리자 페이지
    1. 회원 전체 목록 조회
      1. 회원 클릭 시 모달을 통해 상세 정보 조회
    2. 닉네임으로 회원 검색
    3. 회원 강제 탈퇴
      1. 탈퇴된 회원의 게시글 및 댓글은 그대로 남고, 회원 정보에 ‘탈퇴한 회원입니다’ 표기
